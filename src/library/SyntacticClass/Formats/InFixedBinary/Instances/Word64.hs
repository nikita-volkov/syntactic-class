...
module SyntacticClass.Formats.InFixedBinary.Instances.Word64 where
...
instance Syntactic (InFixedBinary Word64) where
...
instance IsString (InFixedBinary Word64) where
  fromString = fromMaybe (InFixedBinary 0) . maybeFromText . fromString

instance Show (InFixedBinary Word64) where
